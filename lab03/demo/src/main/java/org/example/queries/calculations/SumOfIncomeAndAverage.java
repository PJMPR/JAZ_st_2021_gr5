...
public class SumOfIncomeAndAverage implements Filter {
    @Override
    public void meetCriteria(Results results, SearchParameters searchParameters) {
        ...
        for (FunctionsParameters v : functionsParameters) {
            ...
            switch (v.getFieldName()) {
                case "income" -> value = results.getItems().stream().mapToDouble(Person::getIncome).sum();
                case "age" -> value = results.getItems().stream().mapToDouble(Person::getAge).sum();
